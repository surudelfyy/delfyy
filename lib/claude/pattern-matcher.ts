import { callClaudeJSON, CLAUDE_MODEL_SONNET } from '@/lib/claude/client'
...
export async function matchPatterns(
...
): Promise<PatternMatcherOutput | null> {
  ...
  const selected =
    topScore === 0
      ? allExamples.slice(0, 25)
      : scored.slice(0, 25).map((s) => s.atom)
  ...
  const referenceNotes = examples.map((a) => ({
    ...
    timeframe:
      a.timeframe &&
      typeof a.timeframe.start_year === 'number' &&
      typeof a.timeframe.end_year === 'number'
  ...
  const runOnce = async (model: string, extraUser?: string) => {
    ...
    const normalized: PatternMatcherOutput = {
      ...raw,
      where_worked: Array.isArray(raw.where_worked)
        ? raw.where_worked.slice(0, 3)
        : [],
      where_failed: Array.isArray(raw.where_failed)
        ? raw.where_failed.slice(0, 3)
        : [],
    ...
    const allEmpty =
      (value.where_worked?.length ?? 0) === 0 &&
      (value.where_failed?.length ?? 0) === 0
  ...
  console.log(
    '[pattern-matcher] sonnet empty -> retrying with stronger instruction',
  )
  ...
  const fallbackWorked = examples.find(
    (e) => (e.outcome ?? '').toLowerCase() === 'worked',
  )
  const fallbackFailed = examples.find(
    (e) => (e.outcome ?? '').toLowerCase() === 'failed',
  )
