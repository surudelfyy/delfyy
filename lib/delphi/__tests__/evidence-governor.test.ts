...
function makeLens(
  overrides: Partial<LensOutput> & { lens?: LensOutput['lens'] },
): LensOutput {
  return {
    lens: overrides.lens ?? 'Customer',
    stance: overrides.stance ?? 'mixed',
    summary: overrides.summary ?? 'summary',
    supporting_points: overrides.supporting_points ?? [
      {
        point: 'p',
        atom_ids: ['a1'],
      },
    ...
    disconfirming_tests: overrides.disconfirming_tests ?? [
      {
        test: 'test',
        pass_signal: 'pass',
        fail_signal: 'fail',
    ...
    examples_in_pack: overrides.examples_in_pack ?? [
      {
        example: 'ex',
        lesson: 'lesson',
        atom_ids: ['a3'],
...
function runGovernor(
  input: Partial<GovernorInput>,
): ReturnType<typeof runEvidenceGovernor> {
...
describe('Evidence Governor', () => {
  ...
  it('unclear stance applies -0.15', () => {
    ...
    expect(out.reasons).toContain(
      "-0.15: at least one lens stance is 'unclear'",
    )
  })
  ...
  it('conflict (support + oppose) applies -0.20 and triggers round 2', () => {
    const out = runGovernor({
      lensOutputs: [
        ...
        makeLens({ stance: 'oppose', lens: 'Business' }),
      ],
    })
  ...
  it('risk_flags downgrade -0.10', () => {
    const out = runGovernor({
      classifierOutput: makeClassifier({ risk_flags: ['r1', 'r2'] }),
    })
  ...
  it('missing disconfirming_tests downgrade -0.10', () => {
    ...
    expect(out.confidence_score).toBe(0.4)
    expect(out.reasons).toContain(
      '-0.10: missing disconfirming_tests in at least one lens',
    )
  })
  ...
  it('plan mode uses same tier mapping as choose', () => {
    ...
    const out = runGovernor({
      ...
      classifierOutput: makeClassifier({
        decision_mode: 'plan',
        risk_flags: ['r1'],
      }),
    })
  ...
  it('missing disconfirming_tests when undefined counts as missing', () => {
    const out = runGovernor({
      lensOutputs: [
        {
          ...makeLens({}),
          disconfirming_tests: undefined as unknown as never,
        },
      ],
    })
    expect(out.confidence_score).toBe(0.4)
