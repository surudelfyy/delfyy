...
export function ConfidencePill({ tier }: ConfidencePillProps) {
  ...
  const config =
    tierConfig[tier as keyof typeof tierConfig] || tierConfig.directional
  return (
    <div className="relative">
      ...
      {showTooltip && (
        <>
          ...
          <div className="absolute right-0 top-10 z-50 w-72 bg-background border border-border rounded-xl shadow-xl p-4">
            <div className="flex items-start justify-between mb-3">
              ...
              <button
                onClick={() => setShowTooltip(false)}
                className="text-muted-foreground hover:text-muted-foreground -mt-1 -mr-1"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
